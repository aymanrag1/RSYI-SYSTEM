...
$permits = $wpdb->get_results(
    $wpdb->prepare(
        "SELECT ep.*, sp.arabic_full_name, sp.id AS profile_id
         FROM {$wpdb->prefix}rsyi_exit_permits ep
         JOIN {$wpdb->prefix}rsyi_student_profiles sp ON sp.id = ep.student_id
         WHERE {$where}
         ORDER BY FIELD(ep.status,'pending_dorm','pending_manager','approved','executed','rejected'), ep.created_at DESC
         LIMIT %d OFFSET %d",
        ...$params
    )
);
...
?>
<h1 class="wp-heading-inline"><?php esc_html_e( 'أذونات الخروج', 'rsyi-sa' ); ?></h1>
<hr class="wp-header-end">

<form method="get" class="rsyi-filter-form">
    <input type="hidden" name="page" value="rsyi-exit">
    <select name="status">
...
<table class="widefat rsyi-table striped">
    ...
    <?php else : ?>
        <?php foreach ( $permits as $p ) :
            $is_pending = in_array( $p->status, [ 'pending_dorm', 'pending_manager' ], true );
        ?>
        <tr>
            ...
            <td>
                <?php if ( $is_pending && current_user_can( 'rsyi_approve_exit_permit' ) ) : ?>
                <button class="button button-small rsyi-exit-approve-btn"
                        data-id="<?php echo esc_attr( $p->id ); ?>"
                        data-confirm-msg="<?php esc_attr_e( 'هل تؤكد الموافقة؟', 'rsyi-sa' ); ?>">
                    ✅ <?php esc_html_e( 'موافقة', 'rsyi-sa' ); ?>
                </button>
                <button class="button button-small rsyi-exit-reject-btn"
                        data-id="<?php echo esc_attr( $p->id ); ?>">
                    ❌ <?php esc_html_e( 'رفض', 'rsyi-sa' ); ?>
                </button>
                <?php elseif ( $p->status === 'approved' && current_user_can( 'rsyi_approve_exit_permit' ) ) : ?>
                <button class="button button-small rsyi-exit-execute-btn"
                        data-id="<?php echo esc_attr( $p->id ); ?>"
